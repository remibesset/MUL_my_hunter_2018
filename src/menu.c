...
void manage_mouse_click_menu(sfRenderWindow *window, game_t game)
{
    ...
    sfMouse_isButtonPressed(sfMouseLeft) == sfTrue) {
        ...
        mouse_position.x <= game.mode.width / 2 - 250 + 200 && mouse_position.y > game.mode.height / 2
        + 80 && mouse_position.y <= game.mode.height / 2 + 130) {
            play(game);
        } 
        else if (mouse_position.x >= game.mode.width / 2 - 250 + 250 && 
        mouse_position.x <= game.mode.width / 2 - 250 + 444 && mouse_position.y > game.mode.height / 2
        + 80 && mouse_position.y <= game.mode.height / 2 + 130) {
...
{
    sfVector2f pos_menu = {game->mode.width / 2 - 250, game->mode.height / 2 - 250};

    game->img_menu.texture = sfTexture_createFromFile("include/menu.png", NULL);
